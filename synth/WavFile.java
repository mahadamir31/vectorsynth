...
//    private File file;                                              // File that will be read from or written to
    ...
    public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
        ...
        wavFile.filename = file.getCanonicalPath();
//        wavFile.file = file;
    ...
    public static WavFile openWavResource(String resourceFilename) throws IOException, WavFileException
        {
        return openWavResource(WavFile.class, resourceFilename);
        }

    public static WavFile openWavResource(Class cls, String resourceFilename) throws IOException, WavFileException
        {
        return openWavStream(cls.getResourceAsStream(resourceFilename), cls.getResource(resourceFilename).toString(), -1);
        }

    public static WavFile openWavFile(File file) throws IOException, WavFileException
        {
        return openWavStream(new FileInputStream(file), file.getCanonicalPath(), file.length() );
        }

    public static WavFile openWavStream(InputStream stream, String name, long len) throws IOException, WavFileException
        {
        // Instantiate new Wavfile and store the file reference
        WavFile wavFile = new WavFile();
        wavFile.filename = name;
        ...
        if (len == -1)
            {
            System.err.println("Wav stream is not a file: will not be able to determine if the size is right, crossing fingers.");
            }
        else if (len != chunkSize+8) 
            {
            throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + len + ")");
            }

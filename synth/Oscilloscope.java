...
/**
   A display for Unitulation signals.
*/
...
public class Oscilloscope extends JComponent
    ...
    private Unit amplitude = new Constant(1.0);
    ...
    public OUnit getUnit() { return new OUnit(); }

    ReentrantLock lock = new ReentrantLock();
                        
    public class OUnit extends Unit
        {
        public void setAmplitudeUnit(Unit amplitude) 
            {
            Oscilloscope.this.amplitude = amplitude;
            }

        public double getAmplitudeUnitOutputAsValue() 
            {
            return getAmplitudeUnit().getValue();
            }
        ...
        public double tick(long tickCount)
            {
            final double val = getAmplitudeUnitOutputAsValue();
            lock.lock();
            try
                {
                count++;
